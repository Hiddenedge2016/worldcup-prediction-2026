...
function renderMatchCard(m) {
  ...
  card.innerHTML = `
    ...
    <div class="match-probs">
      <span>🏠 <strong style="color:var(--teal)">${m.winProb.homeWin}%</strong></span>
      <span>🤝 <strong style="color:var(--muted)">${m.winProb.draw}%</strong></span>
      <span>✈️ <strong style="color:var(--coral)">${m.winProb.awayWin}%</strong></span>
      <span>🎯 预测最可能 ${m.bestScore} (${m.bestProb}%)</span>
    </div>
    <div class="top5-scores">
      <span style="color:var(--lobster);font-weight:600">Top 5</span>
      ${m.top5.map(s => `<span class="score-item">${s.score} <span style="color:var(--muted)">${s.prob}%</span></span>`).join('')}
    </div>
  `;
  return card;
}
...
function renderRealStandings(groupCode) {
  ...
  sorted.forEach((s, i) => {
    ...
    html += `<tr class="${qClass}">
      ...
      <td style="text-align:center;color:var(--teal)">${s.w}</td>
      <td style="text-align:center;color:var(--muted)">${s.d}</td>
      <td style="text-align:center;color:var(--coral)">${s.l}</td>
      <td style="text-align:center">${s.gf}:${s.ga}</td>
      <td style="text-align:center;font-weight:${s.gd > 0 ? '700' : '400'};color:${s.gd > 0 ? 'var(--teal)' : s.gd < 0 ? 'var(--coral)' : 'var(--muted)'}">${s.gd > 0 ? '+' : ''}${s.gd}</td>
      <td style="text-align:right;font-weight:700;color:var(--lobster)">${s.pts}</td>
    </tr>`;
  });
  ...
  if (isComplete) {
    html += `<div class="legend-row">
      <span class="legend-dot" style="background:var(--teal-dim);border:1px solid var(--teal)"></span> 前二出线区
    </div>`;
  }
...
function renderStandings(standings, groupCode) {
  ...
  standings.forEach((s, i) => {
    ...
    const barColor = pct > 70 ? 'var(--teal)' : pct > 40 ? 'var(--lobster)' : 'var(--muted)';
...
function renderAccuracyStats() {
  ...
  if (total === 0) return `<div class="section"><div class="section-header"><h2>📊 预测准度</h2></div><p style="color:var(--muted);font-size:14px">暂无已完赛比赛，数据积累中...</p></div>`;
...
function renderPanels() {
  ...
  groups.forEach((g, i) => {
    ...
    desc.style.cssText = 'color:var(--muted);font-size:13px;margin-bottom:16px';
...
function setupNav() {
  document.querySelectorAll('.nav-link').forEach(link => {
    link.addEventListener('click', (e) => {
      ...
      ['groups', 'knockout', 'daily', 'comments', 'about'].forEach(id => {
...
function renderNewsSection() {
  ...
  if (!container || !news || !news.headlines || news.headlines.length === 0) return;
  
  const sorted = [...news.headlines].sort((a, b) => new Date(b.date) - new Date(a.date)).slice(0, 8);
  
  let html = '<div class="fun-grid">';
  ...
  sorted.forEach(item => {
    ...
    html += '<a href="news.html?id=' + item.id + '" class="fun-card" data-id="' + item.id + '">' +
...
function renderAccuracySection() {
  ...
  if (!results) {
    container.innerHTML = '<p style="color:var(--muted);font-size:14px">暂无已完赛比赛，数据积累中...</p>';
    return;
  }
  ...
  Object.keys(data).forEach(function(g) {
    ...
    grp.matches.forEach(function(m) {
      var real = getRealScore(g, m.home, m.away);
      if (!real || real.status !== 'finished') return;
      total++;
  ...
  var accDir = (outcome / total * 100).toFixed(0);
  var accSum = ((exact + outcome) / total * 100).toFixed(0);
  
  container.innerHTML = '<div class="accuracy-stats-grid">' +
    '<div class="acc-stat-card"><div class="acc-stat-num total">' + total + '</div><div class="acc-stat-label">已完赛</div></div>' +
    '<div class="acc-stat-card"><div class="acc-stat-num exact">' + exact + '<span style="font-size:14px;color:var(--muted)">/' + total + '</span></div><div class="acc-stat-label">精准命中</div></div>' +
    '<div class="acc-stat-card"><div class="acc-stat-num good">' + outcome + '<span style="font-size:14px;color:var(--muted)">/' + total + '</span></div><div class="acc-stat-label">方向正确</div></div>' +
    '<div class="acc-stat-card"><div class="acc-stat-num miss">' + miss + '<span style="font-size:14px;color:var(--muted)">/' + total + '</span></div><div class="acc-stat-label">偏差</div></div>' +
    '</div>' +
    '<div style="text-align:center">' +
    '<div style="display:flex;gap:8px;justify-content:center;flex-wrap:wrap;margin-top:4px">' +
    '<span style="background:var(--teal-dim);color:var(--teal);padding:4px 12px;border-radius:6px;font-size:12px;font-weight:700">🎯 精准 ' + accExact + '%</span>' +
    '<span style="background:var(--lobster-dim);color:var(--lobster);padding:4px 12px;border-radius:6px;font-size:12px;font-weight:700">✅ 方向 ' + accDir + '%</span>' +
    '<span style="background:var(--surface2);color:var(--muted);padding:4px 12px;border-radius:6px;font-size:12px;font-weight:700">📊 总正确 ' + accSum + '%</span>' +
    '</div></div>';
}
  ...
  if (!ok) {
    document.getElementById('loader').innerHTML = `
      <div style="text-align:center">
        ...
        <p style="color:var(--muted);margin-top:16px">加载预测数据失败了，请稍后再试</p>
      </div>`;
    return;
  }
